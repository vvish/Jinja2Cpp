...
struct JsonInserter : visitors::BaseVisitor<rapidjson::Value>
{
    ...
    JsonInserter(rapidjson::Document::AllocatorType& allocator)
    ...
    rapidjson::Value operator()(const std::string& str) const
    {
        return rapidjson::Value(str.c_str(), m_allocator);
    }

    rapidjson::Value operator()(const nonstd::string_view& str) const
    {
        return rapidjson::Value(str.data(), str.size(), m_allocator);
    }
    ...
    rapidjson::Value operator()(bool val) const
    {
        return rapidjson::Value(val);
    }

    rapidjson::Value operator()(EmptyValue) const
    {
        return rapidjson::Value();
    }

    rapidjson::Value operator()(const Callable&) const
	{
        return rapidjson::Value("<callable>");
	}

    rapidjson::Value operator()(double val) const
    {
        return rapidjson::Value(val);
    }

    rapidjson::Value operator()(int64_t val) const
    {
        return rapidjson::Value(val);
    }
